...
?>
    ...
    <p>
        ...
        if (isset($_GET['table'])) {
            ...
            $idColumn = 'id';
            ?>
            <a href="lab6.php<?= $urlBack ?>">Back</a>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ?>
                <a href="lab6.php?table=<?= current($row) ?>"><?= current($row) ?></a>
                <br>
                <?php
            }
        }
...
if (isset($_GET['action']) && ($_GET['action'] == 'create' || $_GET['action'] == 'update') || $_GET['action'] == 'delete') {
    ?>
    <form action="lab6.php?table=<?= $_GET['table'] ?>" method="post">
    ...
    if ($_GET['table'] && $ifExist = mysqli_query($conn, "SHOW TABLES FROM supply LIKE '{$_GET['table']}'")) {
        ...
        <p>
            <b><?= variableToName($_GET['table']) ?></b>
            <a href="lab6.php?action=create&table=<?= $_GET['table'] ?>">
                Create entry
            </a>
        </p>
        <table>
            <tr>
                ...
                while ($column = mysqli_fetch_assoc($columns)) {
                    ...
                    ?>
                    <th>
                        <a href="lab6.php?table=<?= $_GET['table'] ?>&sort=<?= $column['Field'] ?>&direction=<?= $_GET['direction'] === 'desc' ? 'asc' : 'desc' ?>">
                ...
                ?>
                <th>Action <a href="lab6.php?table=<?= $_GET['table'] ?>"> reset</a></th>
            </tr>
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <tr>
                    ...
                    foreach ($arrColumns as $column) {
                        ...
                        <td>
                            ...
                            if ($foreign && $foreign['name_column'] == $column) {
                                ?>
                                <a href="lab6.php?<?= "table={$foreign['table']}&where_column={$foreign['where']}&where={$row[$column]}" ?>"><?= $row[$column] ?></a>
                                <?php
                            } else {
                                ?>
                                <a href="lab6.php?<?= "table={$_GET['table']}&where_column={$column}&where={$row[$column]}" ?>">&#x268C;</a>
                                <?php
                                echo $row[$column];
                            }
                            ?>
                    ...
                    ?>
                    <td>
                        <a href="lab6.php?action=update&id_column=<?= $idColumn ?>&<?= $idColumn ?>=<?= $row[$idColumn] ?>&table=<?= $_GET['table'] ?>">Update</a>
                        <a href="lab6.php?action=delete&id_column=<?= $idColumn ?>&<?= $idColumn ?>=<?= $row[$idColumn] ?>&table=<?= $_GET['table'] ?>">Delete</a>
                    </td>
                </tr>
                <?php
            }
